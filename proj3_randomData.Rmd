...
for(cur_mon in 1:10){
  ...
  d[month == cur_mon, sub := ifelse(d$sub[d$month == cur_mon - 1] == 0 & #if a player was not a subscriber last month
                                    ...
                                    #then they have a 10% likelihood of subscribing
                                      sample(c(0,1), size = .N, prob = c(.1,.9), replace = TRUE),
                                    #else if the player was a subscriber last month and then played more than 14 hours
                                      ifelse(d$sub[d$month == cur_mon - 1] == 1 & 
                                               d$mon_hrs_played[d$month == cur_mon - 1] > 14,
                                             #they have a 2% chance of unsubscribing
                                             sample(c(0,1), size = .N, prob = c(.02,.98), replace = TRUE),
                                             #else if they played less 14 hours, either as a subscriber or non 
                                             #subscriber then a player has a 30% of subscribing
                                             sample(c(0,1), size = .N, prob = c(.7,.3), replace = TRUE)))]
  #account closed
  d[month == cur_mon, acct_open := ifelse(d$acct_open[d$month == cur_mon-1] == 1 &  
                                          ...
                                          ifelse(d$acct_open[d$month == cur_mon - 1] == 0, 
                                                 sample(c(0,1),size = .N, prob = c(.98, .02), replace = TRUE),
...
write.csv(d, 'sample_data_v3.csv')
